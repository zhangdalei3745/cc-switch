...
| 打开官网登录并自动取得凭据 | 有条件可实现 | 必须有官方 OAuth、Device Flow、回调或票据交换协议；普通浏览器登录后 CC Switch 无权直接读取浏览器 Cookie |
| 不经过 JoyCode 本地网关直接使用 | 不可行 | 各客户端都不会完整发送 JoyCode 所需的 `ptKey`、`loginType`、签名 URL 和客户端字段 |

因此建议把实现拆成三条线：“共享 JoyCode 协议内核”“九应用接入”“官方登录”。共享内核和应用接入可在地址与凭据契约明确后开发；网页登录必须先拿到 JoyCode 官方授权协议，不能用抓取浏览器 Cookie 的方式代替。
...
4. 登录完成后只展示脱敏账号或“已登录”，不回显完整凭据。
5. “刷新模型”必须在已登录后可用；401 时把状态切换为“凭据失效”。
6. 登录账号和模型列表由共享 JoyCode Auth/Model Store 提供；在一个应用完成登录后，其他应用无需重复登录，只需选择同一账号。
7. 各应用仍独立保存默认模型、模型别名和供应商启用状态，不能因为共享账号而联动切换当前供应商。

## 7. 登录与凭据获取

### 7.1 首选：JoyCode 官方授权协议

只有 JoyCode 提供以下任一机制时，“打开官网登录后自动取得凭据”才能可靠实现：
...
2. **内网、外网各自的官方登录 URL 是什么？** 是否都必须使用 HTTPS？
3. **网页登录后如何把凭据交给 CC Switch？**
   - OAuth/Device Flow 的授权、令牌和发现端点？
   - client ID、scope、redirect URI 白名单？
   - 回调返回授权码、一次性票据还是 `ptKey`？
4. **第三方桌面应用是否被允许获取和保存 `ptKey`？** 有无官方 SDK、有效期、刷新、吊销接口？
5. **登录类型规则是否仍为 `BJ.* → ERP`、其他 → `N_PIN_PC`？** 是否存在更多类型？
6. **外网签名契约是否允许复用？** functionId、签名算法、签名材料、时间容差和轮换策略需要官方确认；不能只复制参考代码中的固定值。
7. **`clientVersion` 的版本策略是什么？** 固定、服务端下发，还是随 JoyCode IDE 版本更新？
8. **是否允许 CC Switch 读取本机 JoyCode/VS Code/JetBrains 凭据作为登录备选？** 若允许，需要确认平台路径、键名和用户授权文案。
9. **JoyCode 是否允许非 JoyCode 客户端经本地协议转换使用服务？** 全应用接入会让 Claude、Gemini、OpenCode 等客户端调用同一服务，需要确认授权和使用政策。
10. **Responses 响应链的正式契约是什么？** `store` 是否强制、response ID 服务端保留多久、允许哪些客户端/账号复用、无效游标的稳定错误码是什么？
11. **缓存如何计费和观测？** 各模型是否返回 `cached_input_tokens` 或等价字段，Responses 链式请求是否按缓存输入计价，缓存写入/读取单价分别是多少？没有该信息无法验收“费用下降”。
12. **是否有官方附件上传与复用 API？** 需要端点、支持 MIME/大小、`file_id` 生命周期、账号/网络作用域、删除方式；未确认前不能把本地 hash 当成服务端可识别引用。
13. **请求体、单附件、单图片和上下文的硬限制是什么？** 内外网网关限制可能不同，需要官方数值和错误码。
